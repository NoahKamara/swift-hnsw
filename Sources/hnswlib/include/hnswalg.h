...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    HierarchicalNSW(
        ...
        level_generator_.seed(static_cast<unsigned int>(random_seed));
        update_probability_generator_.seed(static_cast<unsigned int>(random_seed + 1));
        ...
        visited_list_pool_ = std::unique_ptr<VisitedListPool>(new VisitedListPool(1, static_cast<int>(max_elements)));
...
//                    if (candidate_id == 0) continue;
    ...
    void resizeIndex(size_t new_max_elements) {
        ...
        visited_list_pool_.reset(new VisitedListPool(1, static_cast<int>(new_max_elements)));
    ...
    size_t indexFileSize() const {
        ...
        for (size_t i = 0; i < cur_element_count; i++) {
            unsigned int linkListSize = element_levels_[i] > 0 ? static_cast<unsigned int>(size_links_per_element_ * element_levels_[i]) : 0;
    ...
    void loadIndex(const std::string &location, SpaceInterface<dist_t> *s, size_t max_elements_i = 0) {
        ...
        visited_list_pool_.reset(new VisitedListPool(1, static_cast<int>(max_elements)));
        ...
        ef_ = 10;
        for (size_t i = 0; i < cur_element_count; i++) {
            label_lookup_[getExternalLabel(static_cast<tableint>(i))] = static_cast<tableint>(i);
            unsigned int linkListSize;
            readBinaryPOD(input, linkListSize);
            if (linkListSize == 0) {
                element_levels_[i] = 0;
        ...
        for (size_t i = 0; i < cur_element_count; i++) {
            if (isMarkedDeleted(static_cast<tableint>(i))) {
                num_deleted_ += 1;
                if (allow_replace_deleted_) deleted_elements.insert(static_cast<tableint>(i));
            }
        }
